...
def accelerometer():
    band.start_raw_data_realtime(accel_raw_callback=accel_l)
    raw_input('Press Enter to continue')
...
def get_steps():
    steps = band.get_steps_miband3()
    if steps == None:
        print("None")
    else:
        print(steps)
    time.sleep(10)
    print("\n Steps number: {}".format(steps['steps']))
...
steps_menu      = FunctionItem("Steps in real time", get_steps)
accel_menu      = FunctionItem("Realtime accelerometer WIP", accelerometer)
...
menu.append_item(steps_menu)
menu.append_item(accel_menu)
menu.show()
